  ...
  const saveScenario = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.status === "saved") {
        ...
        setStatus("⚠️ Сценарий с таким названием уже есть в библиотеке");
      }
    } catch (error) {
  ...
  return (
    <main className={styles.container}>
      ...
      <form className={styles.form} onSubmit={saveScenario}>
        ...
        <div className={styles.actions}><Button type="submit" variant="success" loading={isSaving} disabled={!title.trim() || !summary.trim()}>Сохранить сценарий</Button>{status && <p className={`${styles.status} ${status.startsWith("❌") ? styles.error : ""}`} role="status">{status}</p>}</div>
      </form>
    </main>
  );
};
