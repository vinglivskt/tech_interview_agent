...
- Name: full-flow.spec.ts >> Quiz mode >> submitting an answer advances to next question with explanation
- Location: tests/e2e/full-flow.spec.ts:91:3
...
> 96  |     await expect(page.getByText(/Вопрос 1 из 20/)).toBeVisible({ timeout: 10000 });
  ...
  186 |     expect(body).toMatchObject({
  187 |       feature: "quiz",
  188 |       correct: expect.any(Number),
  189 |       partial: expect.any(Number),
  190 |       incorrect: expect.any(Number),
  191 |       total: expect.any(Number),
  192 |     });
  193 |   });
  194 | 
  195 |   test("returns 400 without X-Username", async ({ request }) => {
  196 |     const resp = await request.get("http://127.0.0.1:8000/api/stats/overview");
```