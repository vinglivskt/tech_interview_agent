...
Личный помощник для подготовки к техническим собеседованиям в формате реального интервью. Прогоняет вас через четыре режима — тестирование, устное собеседование, системный дизайн и свободный диалог — и оценивает каждый ответ глазами LLM-эксперта. Стэк: **Python 3.12 / FastAPI + Qdrant (RAG) + Ollama + PostgreSQL** на бэкенде и **React 18 / Vite / TypeScript** на фронтенде. Из LangChain-экосистемы задействованы **LangGraph** (граф дизайн-интервью + чекпоинтер), **LangSmith** (трассировка LLM-вызовов) и **LangChain** (структурный вывод и сплиттер) — точечно, без миграции ядра.
...
- **Точечная интеграция LangChain-экосистемы**: LangGraph-граф дизайн-интервью с персистентным чекпоинтером, LangSmith-трассировка каждого LLM-вызова, структурный вывод (JSON-схема Ollama) и сплиттер через LangChain. Всё включается флагами — офлайн-режим сохраняется.
- **Библиотека из 132 карточек системного дизайна** по 19 категориям и трём уровням, с автогенерацией интервью-лесенки.
...
Каждый вызов LLM во всех режимах (чат, квиз, собеседование, системный дизайн — включая
структурный скоринг `llm.generate_structured`) пишется в LangSmith как run с метаданными
фичи и `session_id`, что делает правку промптов «видящей» (см.
`plans/PLAN_LANGSMITH.md`; скэффолд эвалов — `backend/src/training/langsmith_eval.py`).
По умолчанию выключено — проект остаётся офлайн-first.
...
### LangChain (структурный вывод и сплиттер)

Скоринг-вызовы (sobes/design/classification) отдают JSON надёжно, двумя уровнями:

- **Уровень A — работает всегда**: `format: "json"` в payload скоринг-вызовов —
  модель гарантированно возвращает JSON.
- **Уровень B — флаг `LLM_STRUCTURED_OUTPUT=true`**: ответ валидирует
  `ChatOllama.with_structured_output(..., method="json_schema")` по Pydantic-схемам
  из `backend/src/core/structured.py` (`ScoreResult`, `DesignScore`, `ClassificationBatch`).
  При сбое модели — автоматический фолбэк на legacy-парсинг с ретраями.

| Переменная | Значение по умолчанию | Описание |
|---|---|---|
| `LLM_STRUCTURED_OUTPUT` | `false` | Включить структурный вывод (уровень B) через `ChatOllama.with_structured_output` |
...
| `design/library.yaml` | Библиотека тем системного дизайна: 132 карточки по 19 категориям и 3 уровням |
...
- **`library.yaml`** — библиотека из **132 карточек** по 19 категориям (kafka, cache, streaming, etc.) и уровням `junior`/`middle`/`senior`. При старте приложение загружает её в PostgreSQL (`design_scenarios`) — карточки видны в `GET /api/design/config`.
